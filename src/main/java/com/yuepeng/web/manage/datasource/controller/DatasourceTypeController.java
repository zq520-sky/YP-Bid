 ...
 */
@Controller
@RequestMapping("/manage/datasource/type")
public class DatasourceTypeController extends BaseController{
     ...
     * @return {@link String}
     */
    @RequestMapping("/exportDatasourceTypeList" + WebConstant.BUSINESS_SUFFIX)
    ...
    @RequestMapping("addDatasourceType" + WebConstant.BUSINESS_SUFFIX)
    @ResponseBody
    public Map<String, Object> addDatasourceType(DatasourceTypeVo datasourceTypeVo) throws Exception{
        Date date = new Date();
        datasourceTypeVo.setCreateUserId(CurrentUtil.getCurrentUser().getUserId().intValue());
        datasourceTypeVo.setUpdateUserId(CurrentUtil.getCurrentUser().getUserId().intValue());
        datasourceTypeVo.setUpdateDate(date);
        datasourceTypeVo.setCreateDate(date);
        boolean result = datasourceTypeService.insertSelective(datasourceTypeVo);
        if (result) {
            logService.addLog(new TSysLog("来源类型管理-新增", "新增来源类型【" + datasourceTypeVo.getDatasourceTypeName() + "】成功 ！", PmStateConstant.LOG_PLATFORM));
        } else {
            logService.addLog(new TSysLog("来源类型管理-新增", "新增来源类型【" + datasourceTypeVo.getDatasourceTypeName() + "】失败！", PmStateConstant.LOG_PLATFORM));
        }
        if (!result) {
     ...
     * @return {@link Map< String, Object>}
     */
    @RequestMapping("/getDatasourceTypes"+WebConstant.NO_AUTH_SUFFIX)
    @ResponseBody
    public Map<String, Object> getDatasourceTypes() throws Exception {
        List<DatasourceTypeVo> datasourceTypes = datasourceTypeService.getDatasourceTypes();
        return this.getResultMap(datasourceTypes);
    }

    @RequestMapping("/getDatasourceTypeInfos"+WebConstant.NO_AUTH_SUFFIX)
    @ResponseBody
    public Map<String, Object> getDatasourceTypeInfos() throws Exception {
        List<DatasourceTreeVo> datasourceTypeInfos = datasourceTypeService.getDatasourceTypeInfos();
        return this.getResultMap(datasourceTypeInfos);
    }

    @RequestMapping("/getDatasourceTypeDetails"+WebConstant.NO_AUTH_SUFFIX)
    @ResponseBody
    public Map<String, Object> getDatasourceTypeDetails() throws Exception {
        List<DatasourceTreeVo> datasourceTypeInfos = datasourceTypeService.getDatasourceTypeDetails();
        return this.getResultMap(datasourceTypeInfos);
    }
