...
var dtd = dom.dtd = (function() {
    ...
    return  _({
        ...
        //不能认为是空的元素
        $isNotEmpty : _({table:1,ul:1,ol:1,dl:1,iframe:1,area:1,base:1,col:1,hr:1,img:1,embed:1,input:1,select:1,textarea:1,link:1,meta:1,param:1,h1:1,h2:1,h3:1,h4:1,h5:1,h6:1}),
...
(function () {
    ...
    var iframeUrlMap = {
       ...
       //'highlightcode':'~/dialogs/highlightcode/highlightcode.html',
        ...
        'docfield':'~/dialogs/docfield/docfield.html',
        'flowno':'~/dialogs/flowno/flowno.html',
        'multiimg':'~/dialogs/multiimg/multiimg.html',
        'sutuimage':'~/dialogs/sutuimg/sutuimg.html'
    };
    ...
    var dialogBtns = {
        ...
        ok:['attachment', 'anchor', 'link', 'insertimage', 'map', 'gmap', 'insertframe', 'wordimage',
            'insertvideo', 'insertframe', 'edittip', 'edittable', 'edittd', 'scrawl', 'template', 'music', 'background', 'docfield','flowno','multiimg','sutuimage']
...
//                                        	}
						...
						if ( cmd=='docfield'  || cmd == 'flowno') {
							ui.label = '<font style="color:red">插入要素</font>';
						}
						if ( cmd=='docfield'  || cmd == 'multiimg') {
//							ui.label = '<font style="color:red">本地图片</font>';
						}
						if ( cmd=='docfield'  || cmd == 'sutuimage') {
//							ui.label = '<font style="color:red">速途图片空间</font>';
						}
                        editorui.buttons[cmd] = ui;
                        editor.addListener('selectionchange', function () {
        ...
        initEditorUI:function () {
            ...
            var popup = new baidu.editor.ui.Popup({
                ...
                _onInputDelButtonClick:function(){
                	this.hide();
                	if(popup.anchorEl){
                		domUtils.remove(popup.anchorEl);
                	}
                },
                //2、扩展，编辑控件
                		...
                		if ( el.tagName == 'INPUT' ) {
                			...
                			if ( type == 'text' ) {
                				if ( c=='Wdate' ) {
                					...
                					editor._auto = el;
                					editor.execCommand("auto");
                				} else if ( c == 'detail' ){
                					editor._detail = el;
                					editor.execCommand("detail");
                				} else {
                					editor._textinput = el;
                			...
                			var c = el.getAttribute("class");
                			if ( c == 'htmleditor' ) {
                				editor._htmleditor = el;
                				editor.execCommand("htmleditor");
                			} 
                			else if ( c == 'chooseUser' ) {
                				editor._userdialog = el;
                				editor.execCommand("userdialog");
                			}
                			else if ( c == 'chooseDept' ) {
                				editor._deptdialog = el;
                				editor.execCommand("deptdialog");
                			}
                			else if ( c == 'chooseRole' ) {
                				editor._roledialog = el;
                				editor.execCommand("roledialog");
                			}
                			else {
                				editor._textarea = el;
            ...
            if (editor.options.imagePopup) {
                editor.addListener('mouseover', function (t, evt) {
                   // evt = evt || window.event;
                    var el = evt.target || evt.srcElement;
                    var ifEl = (el.tagName == 'INPUT' || el.tagName == 'TEXTAREA' || el.tagName == 'SELECT');
                    var ifParentEl = (el.tagName == 'SPAN' && el.getAttribute("name")=='form-control');
                    if ( ifParentEl ) {
                    	el = el.childNodes[0];
                    }
                    //扩展
                    if ( el.tagName == 'INPUT' || el.tagName == 'TEXTAREA' || el.tagName == 'SELECT' ) {
                    	...
                    	var tpl = '<nobr>'+editor.getLang("property")+': <span onclick="$$._onInputDelButtonClick()" class="edui-clickable">'+editor.getLang("delete")+'</span>&nbsp;&nbsp;' +	                       
                    	...
                    	if ( readonly ) {
                    		tpl += '<span onclick="$$._unReadOnlyClick(event, this);" class="edui-clickable">删除只读</span>&nbsp;&nbsp;';
                    	} else {
                    		tpl += '<span onclick="$$._onReadOnlyClick(event, this);" class="edui-clickable">设为只读</span>&nbsp;&nbsp;';
                    	}
                    	tpl += '</nobr>';
                    	var html = popup.formatHtml(tpl);
