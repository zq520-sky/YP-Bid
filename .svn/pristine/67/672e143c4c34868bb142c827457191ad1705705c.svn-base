...
function viewPage(id) {
    if (Number(id)) {
        $.ajax({
            url: ctx.path + '/manage/planproject/info/viewPlanproject' + ctx.bizSuffix,
            async: true,
            dataType: 'json',
            type: 'POST',
            data: {planProjectId: id},
            success: function (data) {
                ...
                $("#viewForm #provinceNameView").html(provinceName + " " + cityName)
    ...
    if (Number(id)) {
        $.ajax({
            ...
            success: function (data) {
                ...
                let provinceName = dataRet['provinceName']
                let cityName = dataRet['cityName']
                $("#editForm #provinceNameEdit").html(provinceName + " " + cityName)
                for (var key in dataRet) {
                    //对页面属性赋值（要求页面id与map的key值保持一致）
...
function delPage(id, code, name) {
    if (Number(id)) {
        var msg = "确定是否删除拟建项目【" + name + "】吗？";
        layer.confirm(msg, {
            icon: 3,
            title: "提示信息"
        }, function (index) {
            $.post(ctx.path + "/manage/planproject/info/delPlanProject" + ctx.bizSuffix,
                {
                    planProjectId: id,
                    projectCode: code,
                    projectTitle: name
    ...
    if (Number(id)) {
        $.ajax({
            ...
            success: function (data) {
                ...
                let planProjectId = dataRet['planProjectId']
                let projectCode = dataRet['projectCode']
                $("#editDetailForm #planProjectIdEditDetail").val(planProjectId)
