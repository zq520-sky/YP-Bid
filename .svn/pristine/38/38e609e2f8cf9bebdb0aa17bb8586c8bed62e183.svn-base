...
public class CommonServiceImpl implements ICommonService {
	...
	@Override
	public Map<String, Object> uploadFileSingle(CommonsMultipartFile file, String path) {
		Map<String,Object> result = new HashMap<String, Object>();
		//上传到服务器路径
		String filePath  = CommonConstant.baseUploadPath + path;
		//得到原文件名
		String originalName = file.getOriginalFilename();
		//得到文件后缀名
		String suffixName = file.getOriginalFilename().substring(file.getOriginalFilename().lastIndexOf("."));
		...
		try {
			file.transferTo(newFile);
			//构建返回值
			String relativePath =  CommonConstant.imgUrl + path + File.separator + newFileName;
			result.put("attachmentName", originalName);
			result.put("attachmentPath", relativePath);
			result.put("attachmentSuffix", suffixName);
			result.put("attachmentSize", fileSize);
